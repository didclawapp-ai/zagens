...
**Zagens** (desktop app in `crates/desktop/`) and the runtime workspace share **`0.9.0`**. Desktop still carries an independent literal in `crates/desktop/Cargo.toml` checked by `check-versions.sh` against Tauri/npm/About. Public releases use `0.MINOR.PATCH` until **1.0.0 GA**. Display form **v** + manifest version (e.g. **v0.9.0**).
...
- **Office → external `zagens-office` (breaking):** Removed built-in Office mode / `write_office` / `read_office` and bundled PBS Python; document workflows use skill **`zagens-office`** + CLI via `exec_shell` (hard route, no script fallback).
- **Desktop streaming stability:** Fixes for multi-turn / restore message-id collisions, mid-turn replace, lossy delta dedup, and dual「生成中」frames.
- **Browser P0 + Windows CDP:** Shared `zagens-browser-policy`, session vs persistent allowlist, CDP interact/snapshot/history on Windows.
- **Windows `exec_shell`:** Host-aware tool description, PowerShell spawn alignment, output spill, optional `[agent] shell`.
...
- **Desktop dual identical bubbles (message id collision after restore):** Reloading a session (thread replay / cache) left rows as `msg-1`, `msg-2`, … while `useTurnSend.nextId()` reset and reminted the same ids for the next prompt. `patchStreamingAssistant` updated *every* row with that id, so the previous and current assistants both showed the new stream (dual「生成中」, identical prose) — especially on an existing thread after installing a new build. Fixes: collision-safe `allocateMessageId`; note restored ids before send; patch only the last matching id and settle older duplicates. Also: bind thread before provisional restore; restore merge that keeps an in-flight follow-up; background stream appends a fresh assistant instead of rebinding; live-layout requires `streaming === true`; clear stale `recoveryCtx` on send.
- **Desktop streaming output replaced mid-turn (C → wrong completed text):** After several turns, a late transcript rebuild can see user C on disk while assistant C has no items yet — so its "last assistant" is still turn B. Merging last-to-last replaced the in-flight C bubble with B's completed output (looks like the stream was swapped for a different answer). Guards: skip reconcile when the replay has fewer assistant bubbles than live; refuse to clobber a streaming bubble with non-overlapping completed prose; preserve `isStreaming` across reconcile; recovery skips `setMessages` when the merge is a no-op.
- **Desktop duplicate assistant message (lossy delta dedup / volume-amplified):** `appendStreamingTextDelta` treated any delta that matched the current text's suffix — or (worse) any mid-string substring — as a replayed chunk. Short token splits (`"100"+"0"`, `"smart"+"art"`) and, after a bubble grows across many tools/turns, 16+ char batches that collide with earlier repeated phrases were silently dropped. The lossy text then failed to match `item.completed`, and `mergeAgentMessageSegment` appended the whole message again. Fixes: replay-dedup only for chunks ≥16 chars and only exact/trailing/prefix-extension (never mid-string `includes`); segment merge replaces a lossy subsequence; `reconcileAssistantBlocks` no longer re-pushes captions already represented in a coalesced live text block.
- **Desktop duplicate stream on 2nd prompt (same session):** Sending a follow-up prompt in an existing thread could re-render the previous turn's output inside the new streaming bubble. Three guards: (1) the finished turn's `timelineState` is now dropped from the stream context registry at turn end and reset at send, and the mid-turn rehydrate path only adopts a registry timeline while that context `isStreaming` — a stale prior-turn timeline can no longer be replayed into the new turn; (2) `reconcileMessagesFromThread` skips persisted replay snapshots that predate the in-flight turn (fewer user messages than live) instead of merging the previous turn's assistant into the fresh streaming bubble; (3) the post-turn transcript sync skips its merge when a newer turn is already streaming.
- **Desktop dual「生成中」frames:** Stale assistant bubbles that still held `running` tool blocks (after send / transcript merge / stream rebind) no longer force a second live layout above the latest user message; inactive assistants are finalized so only the active stream target stays in-flight.
...
- **`[agent] shell` config:** Optional `agent.shell = "auto" | "pwsh" | "powershell" | "cmd"` in `config.toml` drives Windows spawn, hooks/gates, and the dynamic `exec_shell` tool description (default: auto-detect).
- **Agent Explore phase shell catalog:** `exec_shell` / wait / cancel / interact no longer get re-deferred by phase-managed catalog shrinking (fixes spurious `tool_search` to load shell tools).

## [0.8.9] - 2026-07-24

**Release highlights**

- **Desktop streaming UX:** Live transcript no longer blanks during deep tool loops; SSE reconnect after transient proxy errors; compact Hold panel + streaming reasoning; throttled Browser preview hints.
- **Harness file-changes card:** Live session edit list with +/- stats in the float stack; click-through to Diff.

### Fixed

- **Desktop streaming transcript (thr_436d):** Live turns no longer look blank during deep tool loops — absorbed reasoning stays visible via streaming activity accordion while P4.6 folding is preserved; background threads now accumulate timeline deltas in the registry; session reattach keeps `turnId` and resumes poll when the SSE controller is gone. Activity rows no longer oscillate expand/collapse when a later tool starts (trailing-row policy).
...
- **Desktop Browser pane (P2 write + preview):** Agent tools `browser_click` / `browser_type` / `browser_scroll` (by snapshot ref; default ask). `browser_start_preview` + UI「起 preview.json」read `.zagens/preview.json` (`command`/`cwd`/`url`/`ready_pattern`), spawn, wait substring ready, navigate. `browser_yolo` / `[browser] yolo` / `ZAGENS_BROWSER_YOLO` decoupled from global YOLO (writes still ask unless set; sidecar restart applies UI toggle).
- **Desktop Browser pane (P1 productization):** Back/forward/reload, loading + security badge, embed mode pref (auto/B/C), persist vs session profile, destroy-on-leave vs keep-background, session host allowlist for agent external https, `allow_private_lan` pref, real `browser_console_tail` via page hook, `browser://state` events; snapshot accepts `includeScreenshot`.
- **Desktop HTML file preview (P0a/P0b):** Workspace `.html` / `.htm` opens on the **preview** tab by default (code tab still available). Relative CSS / images / scripts in the HTML are rewritten into `data:` URLs from the workspace (bounded); optional `allow-scripts` sandbox (no `allow-same-origin`).
...
- **Desktop streaming timeline — clamp + denser collapse (P4.5):** Reasoning preview capped (~3–4 lines); long user prompts clamp with bottom fade (`UserPromptClamp`); caption absorb raised to 280 chars; plan chips and consecutive failed shells collapse; sample `thr_9409` ~116 spine → ~34 presentation rows.
- **Desktop streaming timeline — activity bundles (P4.6):** Completed thinking between tools is absorbed into the following activity (kills 推理↔执行 flicker); explore/write/shell/plan may merge across those gaps as `mixed` with category-count labels; absorbed reasoning remains expandable inside the collapsed run.
- **Desktop streaming timeline — activity row polish:** Drop redundant “click to expand / N reasoning” hints (chevron suffices); append last file basename or shell command after the count summary.
...
- **Desktop web UI (multi-session P1 wrap-up):** 8s reconcile walks every in-flight `streamingThreadIds` entry (background threads refresh registry transcript via replay); `createSetMessagesForView` routes active-view writes into `StreamContext` (registry SSOT); background turn completion toast with optional switch-to-session action and per-session system notification when another session is focused; detach toast when leaving a streaming session.
...
- **Desktop / Runtime (SenseNova · V4):** SenseNova 托管的 `deepseek-v4-*` 按 **模型 id** 沿用主模型 384K 输出预算与 thinking 参数；原生 `sensenova-*` 仍按 catalog `max_output_length` 封顶 65536，修复误用 384K 导致的 `max_tokens` 400。
- **Desktop web UI (multi-session):** Session strip no longer sticks on spinner after the active turn ends when `streamingThreadIds`/registry lag behind composer unlock — reconcile clears stale controllers, `markThreadStreamIdle` seeds registry context before patch, and sidebar derives from composer `streaming` for the active session.
- **Desktop web UI (multi-session):** Follow-up prompts no longer spawn duplicate sidebar rows when the prior turn finished but the strip spinner was stuck — send path re-resolves the runtime thread from ref/registry/`resume-thread`, clears stale in-flight thread ids before send, and `turn_started` syncs `resumedThreadIdRef` immediately.
- **Desktop web UI (multi-session):** Session strip spinner now clears on turn complete (`StreamContext.isStreaming` synced in `completeStreamUi`); follow-up prompts in the same thread no longer spawn duplicate sidebar rows (serialized `persist-session` per thread + backend claims orphan session hints missing `runtime_thread_id`).
...
- **Desktop web UI (multi-session P0.6 hardening):** `panelChannel` dispatchers (`dispatchPanelChecklist` / `dispatchPanelContext` / `dispatchPanelTaskGraph` / `dispatchPanelScratchpad` / `dispatchHarnessCycleAdvanced`) now accept an optional `originThreadId` and silently drop events whose origin differs from the registered active thread — defensive guard against future call sites that forget the `isBackground` early-return; `useTurnSend` syncs the active thread via `setPanelActiveThreadId` on `activeThreadId` change. `StreamContext.controller` dead field removed (`useTurnStream.streamControllersRef` is the sole SSOT). `multiSession.selfcheck.ts` extended with 6 panelChannel thread-isolation assertions.
- **Desktop (runtime_proxy tests):** `arm_sse_cancel_*` tests no longer intermittently fail under parallel `cargo test` — each test uses a unique window-label prefix so the shared global `SSE_CANCEL_FLAGS` map is not purged mid-test by a sibling test.
- **Desktop web UI (multi-session cross-talk fix):** Eliminated two A/B parallel-session cross-talk risks: (1) `__pending__` shared controller key replaced with per-send unique `__send_N__` tokens — one send completing can no longer abort/delete another concurrent pending send's `AbortController`; `handleCancelStream` (Esc) resolves the controller via `pendingSendKeyRef` instead of a shared fallback. (2) `activeSessionIdRef.current` in persist/migrate paths replaced with a send-time `ownerSessionId` closure — if the user switches from session A to B between A's send and A's `turn_started`, A's draft/messages are no longer migrated to B's session. `multiSession.selfcheck.ts` extended with per-send key isolation and ownerSessionId closure isolation assertions.
