...
- **Architecture / D6 Phase B 方案 B（2026-05-26）：** 接受 CLI + ratatui TUI 退场、**runtime 单 crate** — [`D6_PHASE_B_CLI_SUNSET.md`](docs/tech/adr/D6_PHASE_B_CLI_SUNSET.md)；B0–B2 完成；**构建**：删除 CLI/TUI 孤儿模块（`eval`、`composer_*`、`mcp_server` 等），`cli/*` 测试 helper 改 `#[cfg(test)]`，恢复 engine/compaction/MCP 等测试用 re-export；`dead_code` crate lint + **`cargo check/test --lib` 与 sidecar 契约测试在 `RUSTFLAGS=-Dwarnings` 下通过**。
