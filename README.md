...
> **Unofficial community project** — not affiliated with DeepSeek Inc. Capabilities below are what this repo ships today; the terminal TUI remains the most feature-complete surface. Desktop parity notes: [docs/desktop/TUI_DS_PICK_GAP.md](docs/desktop/TUI_DS_PICK_GAP.md).
...
Features you interact with in the **v0.3.0** window (see [CHANGELOG.md](CHANGELOG.md)):
...
**Versions:** DS Pick desktop **v0.3.0** (`crates/desktop/`); workspace CLI/TUI crates **0.8.15** (root `Cargo.toml`).
...
> **非官方社区项目**，与 DeepSeek 公司无关。功能以本仓库 **v0.3.0** 为准；终端 TUI 仍是最完整交互面。桌面差距：[TUI_DS_PICK_GAP.md](docs/desktop/TUI_DS_PICK_GAP.md)。详细英文表见 [What Makes This Project Different](#what-makes-this-project-different)。
...
**v0.3.0** 窗口内已具备：多会话聊天（流式/停止/思考/上下文条）、工作区预览与 **diff2html**、会话回放、子代理与清单侧栏、任务与技能（**定时自动化列表未展示**）、MCP/路由/用量/系统设置、托盘与通知、中/英 UI。
...
**版本：** DS Pick 桌面 **v0.3.0**；workspace CLI/TUI **0.8.15**（根 `Cargo.toml`）。
