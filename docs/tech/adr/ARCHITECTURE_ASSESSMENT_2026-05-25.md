...
- [ ] **Engine struct 在 core**（M-series 进行中：M1 + M2 + M3 + M4 已落地 — M1: `Op` / `EngineHandle` / `ThreadContextSnapshot` 入核 + `impl TurnEnginePort for EngineHandle<P,R>` core 侧实现；M2: lean `core::engine::config::EngineConfig` (25 字段) + tui `EngineConfigExt` (8 字段) 类型桩立起，facade `lean()` / `ext()` / `into_parts()` 访问器到位 — `Engine::new(slim, ext)` 签名切换留待 M7；M3: `deepseek_core::engine::hosts::{LspHost, SubAgentHost, ShellHost, SandboxHost}` 四个边界 trait 立起（call-graph driven，2 + 3 + 0 + 1 个方法），`DiagnosticBlock` / `SandboxBackend` trait 入核，tui 侧 `LspManager` / `Engine` / `TuiSandboxHost` / `TuiShellHost` 上 `impl` 接线 — `SubAgentSpawnPort` 重命名为 `SubAgentHost`（保留 1-cycle deprecated alias）；**M4**: `McpHost` trait 立起（4 个 default-impl 方法 `is_mcp_tool` / `tool_is_parallel_safe` / `tool_is_read_only` / `tool_approval_description` 委派到 core dispatch 自由函数）— `TurnLoopMcpPool` 空 marker 升级为 named trait + 1-cycle deprecated alias + blanket impl；硬约束达成（spike §6 M4）：`crates/tui/src/mcp.rs` 2218 LOC body 零修改；`McpPoolPort::execute_tool`（P2 PR4 dispatch port）保持正交不动；M5→M8 待启动，见 [`PR_M0_ENGINE_STRUCT_TO_CORE_SPIKE.md`](./PR_M0_ENGINE_STRUCT_TO_CORE_SPIKE.md) §6）
...
| **D5** | **Engine struct → core（M-series M1→M8）** | 4-6 周 / 1 人 | 🟡 **进行中**：[`PR_M0_*`](./PR_M0_ENGINE_STRUCT_TO_CORE_SPIKE.md) — **M1 ✅ 2026-05-25**（`Op`/`EngineHandle`/`ThreadContextSnapshot` 入核 + `impl TurnEnginePort` core 端泛型化，net +99 LOC、§7.4 8/8 测试 + sidecar contract 全绿）+ **M2 ✅ 2026-05-25**（lean `core::engine::config::EngineConfig` 25 字段 + tui `EngineConfigExt` 8 字段类型桩；tui `EngineConfig` 保留 facade 平铺、caller 零修改；新增 `lean()`/`ext()`/`into_parts()`/`from_parts()` 访问器 + 2 round-trip 单测，net +378 LOC，36 core error_taxonomy + `engine_llm_client_override_runs_mock_turn` + sidecar contract + web-ui f3 全绿；`Engine::new(slim, ext)` 双参签名留到 M7）+ **M3 ✅ 2026-05-25**（`deepseek_core::engine::hosts::{LspHost, SubAgentHost, ShellHost, SandboxHost}` 四个边界 trait 立起 — call-graph driven，2+3+0+1 个方法；`DiagnosticBlock` + `SandboxBackend` trait 入核；tui inline `impl` + `TuiSandboxHost`/`TuiShellHost` newtype 接线；`SubAgentSpawnPort` 重命名 + 1-cycle deprecated alias；net ~+320 LOC，`core --lib lsp/sandbox` 11+2 + `tui --lib tools::subagent` 108/108 + 全 §6 回归块 + sidecar contract 全绿）+ **M4 ✅ 2026-05-25**（`McpHost` trait 立起 — 4 个 default-impl 方法 `is_mcp_tool`/`tool_is_parallel_safe`/`tool_is_read_only`/`tool_approval_description` 委派 core dispatch 自由函数；空 marker `TurnLoopMcpPool` 改 `#[deprecated]` alias + 1-cycle blanket impl from `McpHost`，`TurnLoopHost::McpPool` 约束改 `McpHost`；硬约束兑现：`crates/tui/src/mcp.rs` 2218 LOC body 零修改 — tui 侧 `impl McpHost for McpPool {}` 一行接线；core/tui 双定义 `is_mcp_tool` 由 cross-verify drift-guard 单测同步保证；`McpPoolPort::execute_tool` dispatch port 保持正交不动；net ~+275 LOC，`core --lib engine::{dispatch,hosts::mcp}` 6/6 + `tui --lib mcp` 36/36 + `tui --lib m4_drift_guard` 2/2 + sidecar contract + protocol_recovery + web-ui f3 全绿；2 个 pre-existing `core::engine::tests::{topic_memory_…, capacity_pre_request_…}` 失败已在 M3 HEAD 独立复现 — 确认与 M4 MCP 路径无关）；M5–M8 排队中 |
