...
| **D5** | **Engine struct → core（M-series M1→M8）** | 4-6 周 / 1 人 | 🟡 **进行中**：[`PR_M0_*`](./PR_M0_ENGINE_STRUCT_TO_CORE_SPIKE.md) — **M1 ✅ 2026-05-25**（`Op`/`EngineHandle`/`ThreadContextSnapshot` 入核 + `impl TurnEnginePort` core 端泛型化，net +99 LOC）+ **M2 ✅ 2026-05-25**（lean `core::engine::config::EngineConfig` 25 字段 + tui `EngineConfigExt` 8 字段类型桩，net +378 LOC）+ **M3 ✅ 2026-05-25**（`LspHost`/`SubAgentHost`/`ShellHost`/`SandboxHost` 四个边界 trait + `DiagnosticBlock`/`SandboxBackend` 入核 + tui newtype 接线，net ~+320 LOC）+ **M4 ✅ 2026-05-25**（`McpHost` trait 4 个 default-impl 方法委派 core dispatch；`TurnLoopMcpPool` 改 `#[deprecated]` alias；硬约束兑现：`crates/tui/src/mcp.rs` 2218 LOC body 零修改；cross-verify drift-guard 同步双定义；net ~+275 LOC）+ **M5 ✅ 2026-05-25**（`SeamHost` 10 方法覆盖完整 layered-context Flash pipeline #159 + opaque `SeamError = Box<dyn Error + Send + Sync>` + `WorkshopHost` 空 marker + `TopicMemoryHost` 2 方法 + settings 移入实现避免 R9 spike crate dep + `ScratchpadStepState` 类型搬核 + 484 LOC `scratchpad_flow.rs` UI/审计 body 保留 tui 侧用 re-export shim；tui inline `impl SeamHost for SeamManager` 10-方法 UFCS 委派 + `impl TopicMemoryHost for TopicMemoryRuntime` 借用规避 + `TuiWorkshopHost` newtype；Engine call-site swap 13 处（`layered_context.rs` 8 + `cycle_hooks.rs` 4 + `message_handlers.rs` 1）；net ~+493 LOC，`core --lib engine::scratchpad_state` 2/2 + `tui --lib seam_manager` 7/7 + `tui --lib compaction` 66/66 + `tui --lib scratchpad` 25/25 + `tui --lib mcp` 36/36 + `tui --lib tools::subagent` 108/108 + sidecar contract + protocol_recovery + web-ui f3 全绿；2 个 pre-existing `core::engine::tests::{topic_memory_…, capacity_pre_request_…}` 失败仍以相同断言行 / 失败模式持续 — 与 M5 的 3-trait + scratchpad diff 无关，capacity_pre_request 那个失败由 M6 直接处理）；M6–M8 排队中 |
