...
| Field | Value |
|-------|-------|
| Git ref | `8b1538a` (full 3×50 + 1.1 MB fixture + A1 hot/cold trim; 2026-05-23) |
| Date | 2026-05-23 |
...
|--------|--------|------|-------|
| Process RSS peak | **29** | MB | Full HTTP @ `8b1538a`, `deepseek-v4-pro`, 50 turn×3 run median; includes 1.1 MB `read_file` fixture turn; `-Gate` PASS vs 28.5 MB @ `10972e4` (+10% threshold 31.4 MB); log `deliverables/runtime-baseline-full-run.log` |
...
| RSS regression gate (A1 transition) | `.\scripts\runtime-longrun-baseline.ps1 -Runs 3 -Gate` — median RSS must not exceed ADR baseline **+10%** (current baseline **29 MB**; override with `-BaselineRssMB` / `-MaxRegressionPct`) |
