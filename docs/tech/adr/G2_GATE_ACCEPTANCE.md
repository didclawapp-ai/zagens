...
| 4 | A+.7 审批回归 | ✅ 自动化 + 手测 | 单测绿；Zagens §2 弹窗批准/拒绝（2026-05-24）；设置四档 + Composer 只读/可勾 |
...
| Zagens 多窗口手测 | ✅ | §3 双窗并行 + 终端不串窗（2026-05-23） |
| §0.4 `/health` | ✅ | `event_schema_version: 2` |
| Stop 中途取消 | ✅ | §5.1 |
| B-L1 CRAFT 手测 | ✅ | G2 §10 全项（2026-05-24） |
...
| §5.1 Stop | ✅ |
| §2 A+.7 审批弹窗 | ✅ 按需审批 + 批准/拒绝（2026-05-24） |
...
| **§12.2 #3** | Zagens 全链路 | ✅ 单窗（§1）；审批 UI 暂缓 |
...
1. ~~**§2 审批 UI**~~ ✅ 2026-05-24（[G2_PR5_MANUAL_SMOKE_CHECKLIST.md](./G2_PR5_MANUAL_SMOKE_CHECKLIST.md) §2、§9.2）。  
