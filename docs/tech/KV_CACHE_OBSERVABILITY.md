...
## Static-prefix fingerprint (Kernel V3 / kernel-v2 M5)
...
turn meta). The same fingerprints are also captured in the **`KernelEvent`** log
(`ModelRequestIssued.request_fp`) for replay and resume coherence — see
[AGENT_KERNEL_V3.md](./AGENT_KERNEL_V3.md).

Compare fingerprint timestamps with hit-rate drops — a changed
