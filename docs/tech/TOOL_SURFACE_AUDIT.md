...
- **[P1] `git.rs:259`/`git_history.rs:447` C4 + C6** — 子进程无超时、阻塞 async;大 diff 先挂起再 40k 截断。
...
- **[P1] C4** — `describe_image.rs:248` blocking reqwest、`diagnostics.rs:165`、`test_runner.rs:86` async 内阻塞。
...
7. **C4 async 阻塞** — git/test/diagnostics/describe_image 改 `tokio::process` 或 `spawn_blocking`。
