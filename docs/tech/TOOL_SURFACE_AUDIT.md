...
| C6 | **子进程/HTTP 无超时 + 响应体全量读** — git/test/office Python/web 抓取无 timeout；web 响应先 `bytes().await` 全读再按上限截断（OOM 风险在截断之前） | 健壮性/边界 | **P1** | `test_runner.rs`、`office_write.rs`、`fetch_url.rs`、`web_run/page.rs`、`git.rs`/`git_history.rs` | **已缓解★** — web 抓取 + `web_search` + `web.run` search 改 `read_body_capped`;`fetch_url`/`web_run`/`web_search` 绑定 `cancel_token`(chunk/redirect 可中断);`run_tests`/`office_write`/git/`grep_files`(120s) 均有超时。**剩**：OpenSandbox cwd 等待后续 |
...
**效率与准确性**
- **[已缓解★] `shell_output.rs`** — `summarize_output` 此前取**头** 3 行 → cargo 输出只显 `Compiling`,尾部 `test result:` 被丢。现改为对全文(含截断时追加的“Preserved summary lines”块)**从尾部反向扫** `is_summary_line`,取最后 ≤3 条高信号行(`test result:`/失败/错误/退出码),无高信号时才退回头部。注:`truncate_with_meta` 保留块仍从尾段首部扫 ≤80 行(海量 `error[` 仍可能占满),后续可再优化。
...
- **[已缓解★] `search.rs` grep 文件扫描** — walk+scan 包进 `spawn_blocking`+120s 超时;`fs::read` 失败单独计 `files_skipped_io`(不再误报 binary);`context_lines` 硬上限 20。
- **[P1] `search.rs`** — 仍非 ripgrep 进程;大 monorepo 内存仍随 walk 线性涨(P2)。
...
- **[已缓解★] C6** — 全部 HTTP 出网工具绑定 `cancel_token`;`web.run` search 对齐 `web_search`(host policy + Bing fallback + 流式读体)。
- **[已缓解★] `web_run/search.rs`** — 现调 `check_host_policy`、DDG 失败/非 200/读失败 fallback Bing、流式读体上限。
...
8. **C6 子进程/HTTP 超时 + kill** — ✅ 全部缓解(含 cancel_token + web.run search 对齐)。
