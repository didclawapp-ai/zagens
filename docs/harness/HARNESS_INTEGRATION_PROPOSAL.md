...
> **位置：** [`docs/harness/`](./README.md)（原 `docs/tech/adr/` 路径见 [重定向 stub](../tech/adr/HARNESS_INTEGRATION_PROPOSAL.md)）

**Status:** Proposed (2026-05-26)
**Owner:** 架构 owner（待签收）
**Source vision:** [`Agent+Harness组合式编程方案.md`](./Agent+Harness组合式编程方案.md) v1.2
**Related:** [`../harness/README.md`](./README.md) · [`ANTHROPIC_MANAGED_AGENTS_AND_HARNESS.md`](./ANTHROPIC_MANAGED_AGENTS_AND_HARNESS.md) · [`../tech/adr/ARCHITECTURE_ASSESSMENT_2026-05-25.md`](../tech/adr/ARCHITECTURE_ASSESSMENT_2026-05-25.md) §1 / §5.1 / §7.1 · [`../tech/adr/B2_INJECTION_ARBITRATION.md`](../tech/adr/B2_INJECTION_ARBITRATION.md) · [`../tech/adr/D6_RUNTIME_SERVER.md`](../tech/adr/D6_RUNTIME_SERVER.md) · [`../tech/adr/D7_PERSISTENCE_UNIFICATION.md`](../tech/adr/D7_PERSISTENCE_UNIFICATION.md) · [`../tech/adr/BACKLOG_LANDLOCK_ENFORCE.md`](../tech/adr/BACKLOG_LANDLOCK_ENFORCE.md)
...
[`Agent+Harness组合式编程方案.md`](./Agent+Harness组合式编程方案.md) v1.2（1670 行）提出十二章组合式 Harness 体系：黑板 / 任务图 / 决策分级（L1–L4）/ 行为监控 / Context Reset / 笔记 / 组合式 Harness / 进化引擎 + 数学基础（粗糙集 / 贝叶斯 / 可能性论 / SPRT / 指数衰减 / 多目标优化）。**该文档是远景设计稿，未与现有 crate 形态对齐。**
...
| **进化引擎** | — | **延期 v1.0 之后** | 见 §7；阶段六升级为 manifest/execpolicy 闭环（方案 v1.3 §3.4） |
| **自适应主动 Harness**（阶段六） | manifest 版本化 + 信号→规则表 + 主动干预 | **v1.0+ 探索** | 依赖 D13 组合式装配器 + D7 决策日志 derived view；**不在 Phase 0–3** |
...
|------|---------|---------|
| 本 ADR 评审 + 签收 | 否 | 不变 |
| `Agent+Harness组合式编程方案.md` 顶部 banner，指向本 ADR 名词映射 | ✅ | 不变 |
...
- **任何方案文档（`Agent+Harness组合式编程方案.md`）的 v1.3+ 更新**（含 §3.4 阶段六），必须同步刷新本 ADR §3 映射表。
