...
**Status:** Design draft **v0.7** + **P0/P1/P2 implemented** + **task-agnostic Layer 2 (§6.5) implemented** + **generic stub/incomplete gate (§6.6) implemented** (panel see §11 below; MicroStack manifest at [`fixtures/harness/microstack-completion-gate.toml`](../../fixtures/harness/microstack-completion-gate.toml)). **enforce source** landed via `CompletionGateConfig::sanitized_for_source(trusted)` defensive guard. **§6.5 adds** two **zero per-task config, one global switch covering all tasks** Layer 2 sources: model `[verify:]` proactive replay + toolchain-detected build/test gates. **§0.1** adds external research grounding for design motivation (grounding signal quality × independence); **§3.1 / §6.7** nail down the "judge vs gap enumerator" boundary, establishing design principles for the adversarial auditor. **Phase 4 macro loop (§4 macro fourth dimension): ✅ shipped** (4a–4e; 4e `lht_long_refactor.toml` baseline). **§6.7 adversarial auditor: ✅ shipped** (opt-in, `[long_horizon.adversarial_audit]`). **`coverage-gate` CLI: ✅ shipped** (`zagens coverage-gate`; fmt/clippy/compile/test/checklist/CRAFT verdict). **CRAFT闭环强化: ✅ shipped** (C1 fix-loop auto-spawn; C3 evidence gate; C4 blackboard rounds; C6 LSP hook; C8 pre-review gate; C9 A/B metrics). **Headless regression CI: ✅ shipped** (P3-3/P3-4/P3-6 全部完成).
...
> **Macro fourth dimension (Phase 4, ✅ shipped ~2026-06):** Large refactor (~15K–20K lines) stacks **LHT implementation segment ↔ CRAFT QA segment ↔ LHT completion segment** macro loop on micro Layer 1–3. Orchestrator (`macro_loop.rs`), `spawn_macro_craft_review`, `auto_continue`, Desktop macro panel nodes (4a–4d) are live; **4e regression baseline** ✅ shipped (`fixtures/harness/lht-eval-arms/lht_long_refactor.toml`; `LongRefactor` preset `auto_continue` fixed). Full spec: [`LONG_HORIZON_CODE_TASKS.md`](./LONG_HORIZON_CODE_TASKS.md) §6 Phase 4. **Product iteration P0–P3** (strict/mismatch/manifest/measurement): same doc **§6 product iteration**. **P3-3/P3-4/P3-6** ✅ shipped (headless regression CI + `zagens coverage-gate`).
...
- 2026-06-14 **Batch A / CRAFT 闭环强化全部完成（L1–L3/L7–L8 + C1/C3/C4/C6/C8/C9）：** ① `zagens coverage-gate` CLI subcommand (§6.1 H2 cross-platform gate: fmt/clippy/compile/test/checklist/CRAFT verdict; `cli/handlers/coverage_gate.rs`). ② Phase 4 全收尾: `lht_long_refactor.toml` 4e 回归基线 + `LongRefactor` preset `auto_continue=true` fix. ③ Headless regression CI: `scripts/ci/harness-regression.sh` + `.github/workflows/harness-regression.yml` (daily + weekly stress). ④ CRAFT fix-loop auto-spawn (C1; `executor.rs` post-hook). ⑤ Reviewer evidence gate (C3; `enforce_reviewer_evidence_gate`). ⑥ Blackboard reviewer/verifier rounds (C4; `blackboard.rs`). ⑦ Pre-review fmt/clippy gate (C8; `run_pre_review_gate`). ⑧ LSP post-edit hook (C6; `craft/lsp_post_hook.rs` → `cargo check` diagnostics → blackboard). ⑨ A/B metrics JSONL (C9; `craft/ab_metrics.rs` + `CRAFT_AB_RUNBOOK.md`).
