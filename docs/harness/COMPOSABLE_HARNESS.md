...
> **Macro fourth dimension (Phase 4, ✅ shipped ~2026-06):** Large refactor (~15K–20K lines) stacks **LHT implementation segment ↔ CRAFT QA segment ↔ LHT completion segment** macro loop on micro Layer 1–3. Orchestrator (`macro_loop.rs`), `spawn_macro_craft_review`, `auto_continue`, Desktop macro panel nodes (4a–4d) are live; **4e regression baseline** (LHT-only vs LHT+1 macro round) remains pending. Full spec: [`LONG_HORIZON_CODE_TASKS.md`](./LONG_HORIZON_CODE_TASKS.md) §6 Phase 4. **Product iteration P0–P3** (strict/mismatch/manifest/measurement): same doc **§6 product iteration**.
