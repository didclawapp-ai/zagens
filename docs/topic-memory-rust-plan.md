# topic-memory-graph → Zagens Rust 重写方案（v2 优化版）

> 复审日期：2026-05-20 · 基于 repo 代码现状（`engine.rs:1934`、`Cargo.toml`、`config.rs` 双层结构）逐段修正

## 1. 概述

将 `topic-memory-graph`（TypeScript，零依赖，~450 行，源自 DidClaw / LCLAW monorepo）用 Rust 重写，作为 Zagens 的新 crate `crates/topic-memory`，嵌入 agent 系统提示词注入管线，实现**对话话题图的增量维护、衰减、情绪感知与记忆段生成**。
...
```markdown
## User Cognitive Map (auto-generated by Zagens · do not edit this section)
...
Config 在 Zagens 中有两层：TOML schema（`crates/config/src/lib.rs`）→ 运行时 config（`crates/tui/src/config.rs`）。新 `[topic_memory]` 遵循相同模式。
...
| 步骤 | 内容 |
|------|------|
| 3.1 | 情绪正则调优（基于实际 Zagens 对话日志批量验证，而非直觉调参） |
| 3.2 | 停用词增补（消除噪音话题） |
...
| 停用词不足导致噪音话题 | 话题列表充斥"我们/可以/需要" | 参考真实日志迭代增补；Phase 3 用 Zagens 对话样本调优 |
