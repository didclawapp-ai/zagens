...
**CI rsync 报 `Broken pipe` / exit 12：** 多为 GitHub Actions → VPS 的 SSH 长连接被中间网络或 `sshd` 掐断。工作流已加 SSH `ServerAliveInterval`、rsync `--timeout`/`--partial`，并自动重试 3 次。若仍偶发失败，在 VPS 检查 `MaxStartups`、`ClientAliveInterval`（`/etc/ssh/sshd_config`），或手动重跑 workflow。
