#!/usr/bin/env bash
# Fails CI if version state is inconsistent across the workspace and
# Cargo.lock. Run on every push/PR so silent drift can't ship.
#
# Checks performed:
...
# 1) Literal versions in crate manifests (desktop is versioned independently).
literals="$(grep -nE '^version = "' crates/*/Cargo.toml | grep -v 'crates/desktop/Cargo.toml' || true)"
if [[ -n "${literals}" ]]; then
