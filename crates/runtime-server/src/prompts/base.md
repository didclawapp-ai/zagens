...
- **Windows / Node.js**: if `npm install` fails with **EPERM/EACCES** on the cache, add a project `.npmrc` with `cache=./.npm-cache` (template: `fixtures/harness/workspace-templates/nodejs-windows/.npmrc`) and retry. If **Jest** fails with **spawn EPERM**, retry with `npm test -- --runInBand`. If **devDependencies** are missing after install (npm v11+), retry with `npm install --include=dev`.
