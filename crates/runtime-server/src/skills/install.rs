...
/// Copy a local skill directory (`SKILL.md` + companion files) into `skills_dir`.
///
/// The source directory must contain a valid `SKILL.md` with `name` and
/// `description` frontmatter. Symlinks are rejected; total copied bytes are
/// capped at `max_size`.
