...
pub use crate::scratchpad::note_quality::{
    build_dimension_balance_hint, validate_cleared_claim, validate_deferred_meta_claim,
};
...
/// Append deferred list to L0 when reviewed ratio is low but accounted passes soft gate.
#[must_use]
pub fn format_deferred_areas_l0_suffix(stats: &CoverageStats, config: &ScratchpadConfig) -> String {
    if stats.deferred_areas.is_empty()
        || stats.reviewed_ratio >= config.coverage_reviewed_warn_ratio
    {
        return String::new();
    }
    let mut lines = String::from("\ndeferred_areas:\n");
    for d in &stats.deferred_areas {
        lines.push_str(&format!("- {}: {}\n", d.id, d.reason_excerpt));
    }
    lines
}
