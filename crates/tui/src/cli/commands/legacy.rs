//! CLI command implementations migrated from main.rs (B3).
...
pub(crate) fn run_eval(args: EvalArgs) -> Result<()> {
...
pub(crate) fn init_tools_dir(tools_dir: &Path, force: bool) -> Result<(PathBuf, WriteStatus, WriteStatus)> {
...
pub(crate) async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    use crate::palette;
    use colored::Colorize;
...
pub(crate) fn run_execpolicy_command(command: ExecpolicyCommand) -> Result<()> {
...
pub(crate) async fn test_api_connectivity(config: &Config) -> Result<String> {
    use crate::client::DeepSeekClient;
...
pub(crate) fn list_sessions(limit: usize, search: Option<String>) -> Result<()> {
    use crate::palette;
    use colored::Colorize;
    use crate::session_manager::{SessionManager, format_session_line};
...
pub(crate) fn run_login(api_key: Option<String>) -> Result<()> {
    ...
    let saved = crate::config::save_api_key(&api_key)?;
...
pub(crate) fn resolve_session_id(session_id: Option<String>, last: bool, workspace: &Path) -> Result<String> {
    if last {
...
pub(crate) fn fork_session(session_id: Option<String>, last: bool, workspace: &Path) -> Result<String> {
    let manager = SessionManager::default_location()?;
    let saved = if last {
...
pub(crate) fn read_patch_from_stdin() -> Result<String> {
    let mut stdin = io::stdin();
    if stdin.is_terminal() {
...
pub(crate) async fn run_mcp_command(config: &Config, command: McpCommand) -> Result<()> {
...
pub(crate) fn parse_sandbox_policy(
    policy: &str,
    network: bool,
...
pub(crate) fn should_use_alt_screen(cli: &Cli, config: &Config) -> bool {
...
pub(crate) fn default_mouse_capture_enabled(terminal_emulator: Option<&str>) -> bool {
    if cfg!(windows) {
        return false;
    }
...
pub(crate) fn try_recover_checkpoint() -> Option<String> {
...
pub(crate) async fn run_interactive(
    ...
    if !cli.skip_onboarding {
        match crate::config::ensure_config_file_exists(cli.config.clone()) {
            Ok(Some(path)) => crate::logging::info(format!(
                "Created first-run config file at {}",
                path.display()
            )),
            Ok(None) => {}
            Err(err) => crate::logging::warn(format!("Failed to create first-run config file: {err}")),
        }
    }
    ...
    if snapshots.enabled {
        crate::session_manager::prune_workspace_snapshots(&workspace, snapshots.max_age());
    }
