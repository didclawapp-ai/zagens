...
use super::*;
    ...
    /// PR5 — `ThreadMessageTurnPort` delegates to the live sidecar engine path.
    #[tokio::test]
    async fn runtime_thread_message_turn_port_delegates_sidecar_turn() -> Result<()> {
        use std::sync::Arc;

        use deepseek_core::{ThreadMessageTurnPort, ThreadMessageTurnRequest};
        ...
        tokio::spawn(async move {
            ...
            let _ = tx_event
                .send(EngineEvent::MessageDelta {
                    index: 0,
                    content: "Sidecar delegated reply".to_string(),
                })
                .await;
            let _ = tx_event
                .send(EngineEvent::MessageComplete { index: 0 })
        ...
        let port = RuntimeThreadMessageTurnPort::new(manager.clone())
            .with_turn_timeout(MOCK_ENGINE_TURN_TERMINAL_TIMEOUT);
        let result = port
            .run_turn(ThreadMessageTurnRequest {
                thread_id: thread.id.clone(),
                input: "hello via port".to_string(),
                cwd: PathBuf::from(thread.workspace.clone()),
                model: thread.model.clone(),
            })
            .await?;

        assert_eq!(result.status, "completed");
        assert!(result.assistant_text.contains("Sidecar delegated reply"));
        Ok(())
    }
