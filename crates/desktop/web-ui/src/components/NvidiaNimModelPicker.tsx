...
import type { NvidiaNimModelEntry, NvidiaNimModelList } from '../types/modelProviders';
import { setNvidiaNimOutputLimits } from '../lib/modelParams';
...
export default function NvidiaNimModelPicker({
  currentModel,
  disabled = false,
  onModelChanged,
}: Props) {
  const { t } = useT();
  const [list, setList] = useState<NvidiaNimModelList | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setLoadBusy(true);
    try {
      const data = await invoke<NvidiaNimModelList>('list_nvidia_nim_models');
      setList(data);
      ...
      setNvidiaNimOutputLimits(limits);
    } catch (err) {
      setList(null);
  ...
  const handleSelect = async (modelId: string) => {
    ...
    try {
      await invoke('set_nvidia_nim_model', { modelId });
      onModelChanged();
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
  ...
  return (
    <div className="space-y-2 border-t border-divider pt-3">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[11px] font-medium text-t-text-secondary">{t('models.nvidiaNimModels')}</p>
        <button
          type="button"
        ...
        >
          {loadBusy ? t('models.nvidiaNimRefreshing') : t('models.nvidiaNimRefresh')}
        </button>
      </div>
      ...
      <input
        ...
        placeholder={t('models.nvidiaNimSearch')}
        disabled={disabled || loadBusy}
      ...
      {loadBusy && !list && (
        <p className="text-xs text-t-text-muted">{t('models.nvidiaNimLoading')}</p>
      )}
      ...
      {list && (
        <div
          ...
          role="listbox"
          aria-label={t('models.nvidiaNimModels')}
        >
          {filtered.length === 0 ? (
            <p className="px-2 py-1.5 text-xs text-t-text-muted">{t('models.nvidiaNimNoMatch')}</p>
          ) : (
            filtered.map((m) => (
              <ModelRow
                key={m.id}
...
function ModelRow({
  ...
  if (model.max_output_length != null) {
    meta.push(t('models.nvidiaNimMaxOutput', { tokens: model.max_output_length.toLocaleString() }));
  } else if (model.context_length != null) {
    meta.push(t('models.nvidiaNimContext', { tokens: model.context_length.toLocaleString() }));
  }
  if (model.owned_by) {
    meta.push(model.owned_by);
  }
    ...
    >
      <span className="flex items-center justify-between gap-2">
        ...
        <span className="flex shrink-0 items-center gap-1.5 text-[10px]">
          {selected && <span>{t('models.nvidiaNimCurrent')}</span>}
