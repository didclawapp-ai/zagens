...
export function scrollWorkspaceRelIntoView(container: HTMLElement | null, rel: string): void {
  if (!container || !rel.trim()) return;
  const norm = normalizePathsForCompare(rel);
  const el = container.querySelector<HTMLElement>(
    `[${WS_REVEAL_ATTR}="${CSS.escape(norm)}"]`,
  );
