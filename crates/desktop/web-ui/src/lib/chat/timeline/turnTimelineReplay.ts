...
function toolNameFromItem(kind: TurnItemRecord['kind'], metadata: unknown): string {
  ...
  for (const item of turnItems) {
    ...
    switch (kind) {
      ...
      case 'command_execution': {
        ...
        const name = toolNameFromItem(item.kind, metadata);
...
export function buildAssistantBlocksForTurn(
  ...
  const hadThinkingLive = eventBlocks.some((b) => b.kind === 'thinking');
  const thinkingInResult = blocks.some((b) => b.kind === 'thinking');
  const thinkingIncomplete =
    !thinkingInResult && hadThinkingLive && itemBlocks.length > 0 && events.length === 0;
