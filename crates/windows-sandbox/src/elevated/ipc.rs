...
pub struct ErrorPayload {
    pub message: String,
    pub code: String,
    /// Raw Win32 error code when the failure came from a denied system call
    /// (e.g. `5` = `ERROR_ACCESS_DENIED`, `1385` = logon-type denial) so the
    /// parent can report `sandbox_denial_code` without parsing stderr text.
...
mod tests {
    ...
    fn spawn_request_round_trip() {
        ...
        assert_eq!(payload.command.len(), 3);
        assert_eq!(payload.cap_sids, vec!["S-1-15-3-1024-1".to_string()]);
    }
}
