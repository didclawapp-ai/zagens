use crate::elevated::ipc::FramedMessage;
use crate::elevated::ipc::IPC_PROTOCOL_VERSION;
use crate::elevated::ipc::Message;
use crate::elevated::ipc::SpawnRequest;
use crate::elevated::ipc::read_frame;
use crate::elevated::ipc::write_frame;
use crate::elevated::runner_pipe::PIPE_ACCESS_INBOUND;
use crate::elevated::runner_pipe::PIPE_ACCESS_OUTBOUND;
use crate::elevated::runner_pipe::connect_pipe;
use crate::elevated::runner_pipe::create_named_pipe;
...
impl RunnerTransport {
    ...
    pub(crate) fn read_spawn_ready(&mut self) -> Result<()> {
        ...
        match msg.message {
            ...
            Message::Error { payload } => Err(runner_error_to_anyhow(&payload)),
...
/// Converts a runner `ErrorPayload` into an error, preserving any structured
/// Win32 denial code reported over IPC (PR-2.13).
pub(crate) fn runner_error_to_anyhow(
    payload: &crate::elevated::ipc::ErrorPayload,
) -> anyhow::Error {
    match payload.win32_code {
        Some(code) => anyhow::Error::new(crate::process::SpawnDenial {
            win32_code: code,
            api: "elevated runner",
        })
        .context(format!("runner error: {}", payload.message)),
        None => anyhow::anyhow!("runner error: {}", payload.message),
    }
}
...
fn connect_pipe_with_timeout(
    ...
    let mut connect_thread = Some(
        thread::Builder::new()
            .name(format!("zagens-runner-connect-{pipe_label}"))
            .spawn(move || {
...
pub(crate) fn spawn_runner_transport(
    ...
    let runner_exe = find_runner_exe(zagens_home, log_dir);
    let runner_cmdline = runner_exe
        .to_str()
        .map(str::to_owned)
        .unwrap_or_else(|| "zagens-command-runner.exe".to_string());
    ...
    if spawn_res == 0 {
        ...
        // Typed denial (PR-2.13): code 1385 (logon type) / 5 (access denied)
        // become `sandbox_denial_code` upstream.
        return Err(anyhow::Error::new(crate::process::SpawnDenial {
            win32_code: err,
