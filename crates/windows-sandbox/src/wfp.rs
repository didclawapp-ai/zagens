...
const PROVIDER_DESCRIPTION: &str = "Persistent WFP provider for Zagens Windows sandbox filters";
const SUBLAYER_NAME: &str = "Zagens Windows Sandbox WFP";
const SUBLAYER_DESCRIPTION: &str = "Persistent WFP sublayer for Zagens Windows sandbox filters";

// WFP identifies persistent providers, sublayers, and filters by stable GUIDs.
// These values are Zagens-owned identities; do not regenerate them unless we
// intentionally want to orphan old objects and create a new WFP namespace.
const PROVIDER_KEY: GUID = GUID::from_u128(0x7a676e7300004001b0b000000000001);
const SUBLAYER_KEY: GUID = GUID::from_u128(0x7a676e7300004002b0b000000000002);

/// Installs the persistent Zagens WFP filters for `account`.
///
...
/// Removes all persistent Zagens WFP objects: every filter, then the
/// sublayer, then the provider (reverse of install order — design §8.5 step 1).
///
/// Missing objects are tolerated so teardown is idempotent. Requires the same
/// elevation as install; intended to run from the elevated setup helper in
/// teardown mode.
pub fn remove_wfp_filters() -> Result<usize> {
    let engine = Engine::open()?;
    let mut transaction = engine.begin_transaction()?;
    ...
    let result = unsafe { FwpmSubLayerDeleteByKey0(engine.handle, &SUBLAYER_KEY) };
    ensure_success_or(
        result,
    ...
    let result = unsafe { FwpmProviderDeleteByKey0(engine.handle, &PROVIDER_KEY) };
    ensure_success_or(
        result,
...
/// Ensures the persistent Zagens WFP provider exists.
...
/// Ensures the persistent Zagens sublayer exists under the Zagens provider.
