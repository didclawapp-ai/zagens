...
///
/// Returns `~/.zagens/snapshots/<project_hash>/<worktree_hash>/`. The
...
/// Used by tests so we never touch the user's real `~/.zagens/`.
...
mod tests {
    ...
    #[test]
    fn snapshot_dir_layout_two_levels_under_zagens() {
        ...
        assert_eq!(iter.next().unwrap().as_os_str(), ".zagens");
