...
//!
//! M6 (Engine-struct strangler step, 2026-05-25): the heavy
//! `CapacityController` body — previously living at
//! `crates/tui/src/core/capacity.rs` (677 LOC) — moves into core in a
//! single atomic step per spike R10 ("single atomic move, delete tui
//! copy in same PR"). Only `capacity_config_from_app` (which depends
//! on the tui-side `crate::config::Config` flat representation) stays
//! tui-side; the tui file becomes a thin re-export shim.
//!
//! ## Scope
//!
//! Owns the full controller surface used by `crates/tui/src/core/engine/capacity_flow/*`:
//!
//!   - [`GuardrailAction`] — decision output (`NoIntervention` /
//!     `TargetedContextRefresh` / `VerifyWithToolReplay` /
//!     `VerifyAndReplan`).
//!   - [`RiskBand`] — coarse failure-probability band
//!     (`Low` / `Medium` / `High`).
//!   - [`CapacityObservationInput`] — per-checkpoint observation
//!     payload (turn-index, model, action / tool / ref counts, context
//!     ratio).
//!   - [`DynamicSlackProfile`] — rolling slack-window aggregate
//!     (`final_slack`, `min_slack`, `violation_ratio`,
//!     `slack_volatility`, `slack_drop`).
//!   - [`CapacitySnapshot`] — per-checkpoint snapshot with `h_hat`,
//!     `c_hat`, `slack`, `profile`, `p_fail`, `risk_band`, `severe`.
//!   - [`CapacityDecision`] — controller decision with reason +
//!     cooldown-blocked flag.
//!   - [`CapacityController`] — stateful controller (slack window +
//!     guardrail runtime state).
//!   - [`decide_policy`] — pure policy mapping snapshot → action.
//!   - [`CapacityControllerConfig`] — settings (already in core
//!     since P2 PR4; M6 just consolidates the surrounding state here).
