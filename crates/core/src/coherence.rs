//! Plain-language session coherence state + capacity-event reducer
//! (P2 PR4 enum entry; M1-deferred reducer landed in M6 2026-05-25).
//!
//! `CoherenceState` (the user-facing ladder enum) has lived here since
//! P2 PR4. M1 (spike row #22) was originally going to bring the
//! reducer (`CoherenceSignal` + `next_coherence_state`) along, but
//! the reducer depends on `GuardrailAction` / `RiskBand` from the
//! `capacity` module — those only landed in core under M6, so M1
//! deferred this part. M6 now closes that loop: the reducer joins
