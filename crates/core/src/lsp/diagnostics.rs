...
mod tests {
    ...
    #[test]
    fn render_blocks_concatenates_multiple_files() {
        let blocks = vec![
            DiagnosticBlock {
    ...
    fn render_blocks_skips_empty() {
        ...
        let rendered = render_blocks(&blocks);
        assert!(!rendered.contains("file=\"empty.rs\""));
        assert!(rendered.contains("file=\"real.rs\""));
    }
}
