...
use crate::engine::context::{
    effective_max_output_tokens, is_context_length_error_message, summarize_text,
    MAX_CONTEXT_RECOVERY_ATTEMPTS, TURN_MAX_OUTPUT_TOKENS,
};
...
pub async fn run_streaming_phase<H: TurnLoopHost>(host: &mut H,
...
let stream = match stream_result {
    ...
    Err(e) => {
        ...
        *turn_error = Some(message.clone());
        let _ = host.tx_event().send(Event::error(ErrorEnvelope::classify(message, true)))
            .await;
        return TurnLoopStreamingPhaseOutcome {
...
loop {
    ...
    while let Ok(steer) = host.rx_steer_mut().try_recv() {
        ...
        let _ = host.tx_event().send(Event::status(format!(
                "Steer input queued: {}",
            ...
            ) && is_stream_failure_retryable(&message)
            {
                ...
                match client.create_message_stream(stream_request.clone()).await {
                    ...
                    Err(retry_err) => {
                        ...
                        let _ = host.tx_event().send(Event::error(ErrorEnvelope::classify(
                                retry_msg, true,
                            )))
                            .await;
                        break;
    ...
    match event {
        ...
        StreamEvent::ContentBlockStart {
            ...
            ContentBlockStart::Text { text } => {
                ...
                {
                    let _ =
                        host.tx_event().send(Event::status(FAKE_WRAPPER_NOTICE)).await;
                ...
                let _ = host.tx_event().send(Event::MessageStarted {
            ...
            ContentBlockStart::Thinking { thinking } => {
                ...
                let _ = host.tx_event().send(Event::ThinkingStarted {
                        index: index as usize,
                    })
                    .await;
            }
        ...
        StreamEvent::ContentBlockDelta { index, delta } => match delta {
            Delta::TextDelta { text } => {
                ...
                if !filtered.is_empty() {
                    current_text_visible.push_str(&filtered);
                    let _ = host.tx_event().send(Event::MessageDelta {
                            index: index as usize,
                            content: filtered,
                        })
                        .await;
            ...
            Delta::InputJsonDelta { partial_json } => {
                    ...
                    && let Some(tool_state) = tool_uses.get_mut(index)
                {
                    ...
                    if let Some(value) = host.parse_streaming_tool_input(&tool_state.input_buffer) {
        ...
        StreamEvent::ContentBlockStop { index } => {
            ...
            match stopped_kind {
                ...
                Some(ContentBlockKind::Thinking) => {
                    let _ = host.tx_event().send(Event::ThinkingComplete {
                            index: index as usize,
                        })
                        .await;
                }
                ...
                && let Some(tool_state) = tool_uses.get_mut(index)
            {
                ...
                let _ = host.tx_event().send(Event::ToolCallStarted {
